...
const BudgetAllocation = ({ channel }) => {
  const { id, budgetAllocation } = channel;
  const { handleChannelChange } = useContext(ChannelsContext);

  return (
    <div className="budget-allocation">
      <div className="label-wrapper">
        <div className="label">Budget Allocation</div>
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
      ...
      <div className="allocation-options h-40">
        {allocations.map((allocation, key) => {
          return (
            <div className="allocation-option" key={key}>
              <input
                type="radio"
                name="allocation"
                id={allocation}
                value={allocation}
                checked={budgetAllocation === allocation}
                onChange={(e) =>
